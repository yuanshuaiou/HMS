...
public class RegisterMedicineView {
    ...
    public static void main(String[] args) {
        EventQueue.invokeLater(new Runnable() {
            public void run() {
                try {
                    RegisterMedicineView window = new RegisterMedicineView();
...
//                    Tools.showMessage("请输入类别！");
//                }
                else{
                    int a= MedicineDao.registerMedicine(mid,mname,mnum,mprice);
                    if(a==0){
                        Tools.showMessage("药品冲突，添加失败！");
                    }
                    else{
                        if(a==1){
